...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    if (token.expires !== null &&
      ...
      return done(error('expired_access_token',
    ...
    self.req.body.hasNewMsg = false
    
    var query = `SELECT msgs.ID from msgs LEFT JOIN accesstokens ON msgs.to = accesstokens.user_id WHERE access_token = '${self.bearerToken}' AND \`read\` = 0;`
    mySqlConnection.query(query, function(dataObj){
      if(dataObj.error) done();
      else{
        self.req.body.hasNewMsg =  dataObj.results.length >= 1  ? 'true_' + Date.now(): 'false_' + Date.now()
        done();
      }
    })
