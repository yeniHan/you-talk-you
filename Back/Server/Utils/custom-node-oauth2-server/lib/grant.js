...
function saveAccessToken (done) {
  ...
  if (this.config.accessTokenLifetime !== null) {
    expires = this.now + this.config.accessTokenLifetime*1000 ;
  }
...
function saveRefreshToken (done) {
  ...
  if (this.config.refreshTokenLifetime !== null) {
    expires = this.now + this.config.refreshTokenLifetime*1000;
  }
...
function sendResponse (done) {
  ...
  //Send user_type, too
  var user_type;
  var query = `SELECT user_type from users LEFT JOIN accesstokens ON users.ID = accesstokens.user_id WHERE access_token = '${self.accessToken}';`
  mySqlConnection.query(query, function (dataObj){
    
     response.user_type = dataObj.error != null || dataObj.results.length === 0 ? null: dataObj.results[0].user_type

     if (self.config.accessTokenLifetime !== null) {
      response.expires_in = self.config.accessTokenLifetime;
    }
  
    if (self.refreshToken) response.refresh_token = self.refreshToken;
